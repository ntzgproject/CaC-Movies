document.addEventListener('submit', function(event) {
    ...
    const peliculaBuscada = event.target.buscar.value; // Obtener la película buscada
    const apiUrl = `https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&query=${encodeURIComponent(peliculaBuscada)}&language=es-ES`;
    const options = {
        method: 'GET',
        headers: {
    ...
    fetch(apiUrl, options)
        ...
        .then(data => {
            ...
            infoPelicula = peliculaBuscada;
            localStorage.setItem("peliculaGuardada", JSON.stringify(peliculas)); 
            if (peliculaBuscada) {
                displayMovies(peliculas, 'peliculas');
            }
        })
